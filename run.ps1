...
# 检查程序文件是否存在
if (-not (Test-Path "target\release\smart-forward.exe")) {
    Write-Host "错误: 找不到程序文件 target\release\smart-forward.exe" -ForegroundColor Red
    Write-Host "请先运行 cargo build --release 编译程序" -ForegroundColor Red
    Read-Host "按任意键退出"
    exit 1
}
